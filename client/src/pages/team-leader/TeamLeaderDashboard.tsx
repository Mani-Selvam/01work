...
export default function TeamLeaderDashboard() {
  ...
  const teamPendingLeaves = allLeaves.filter(
    leave => teamMemberIds.includes(leave.userId) && leave.status === 'pending'
  );

  const today = new Date().toISOString().split('T')[0];
  const teamTodayAttendance = todayAttendance.filter(
    record => teamMemberIds.includes(record.userId) && record.date === today
  );
  ...
  const getTodayStatus = (memberId: number) => {
    ...
    if (!record) return { status: 'absent', label: 'Not Checked In', color: 'text-muted-foreground' };
    if (record.status === 'leave' || record.status === 'approved_leave') return { status: 'leave', label: 'On Leave', color: 'text-blue-600 dark:text-blue-400' };
    if (record.checkIn && !record.checkOut) return { status: 'present', label: 'Checked In', color: 'text-green-600 dark:text-green-400' };
    if (record.checkIn && record.checkOut) return { status: 'completed', label: 'Checked Out', color: 'text-muted-foreground' };
    return { status: 'absent', label: 'Absent', color: 'text-red-600 dark:text-red-400' };
  };
            ...
            {teamMembers.length === 0 ? (
              ...
              <div className="space-y-3">
                {teamMembers.map((member) => {
                  const statusInfo = getTodayStatus(member.id);
                  const memberTasks = teamPendingTasks.filter(t => t.assignedTo === member.id);
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          {memberTasks.length > 0 && (
                            <span className="text-xs text-muted-foreground">
                              {memberTasks.length} pending task{memberTasks.length !== 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            <div className="space-y-4">
              {teamPendingLeaves.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-sm font-medium">Pending Leave Requests</h4>
                  <div className="space-y-2">
                    {teamPendingLeaves.slice(0, 3).map((leave) => {
                      const member = teamMembers.find(m => m.id === leave.userId);
                      return (
                        <div 
                          key={leave.id} 
              ...
              {teamPendingTasks.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {teamPendingTasks.slice(0, 3).map((task) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Assigned to: {member?.displayName || 'Unknown'}
                            </p>
                          </div>
                          <Badge 
              ...
              {teamPendingLeaves.length === 0 && teamPendingTasks.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    All tasks and requests are up to date
                  </p>
                </div>
              )}
            </div>
