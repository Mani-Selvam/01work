...
export default function TeamLeaveApproval() {
  ...
  const teamLeaves = allLeaves.filter(leave => teamMemberIds.includes(leave.userId));
  ...
  const rejectLeave = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to reject leave", variant: "destructive" });
    },
  });
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <CardTitle className="text-lg flex items-center gap-2">
                      ...
                      {member?.displayName || 'Unknown Employee'}
                    </CardTitle>
      ...
      {filteredLeaves.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            <p className="text-muted-foreground">No {statusFilter !== "all" && statusFilter} leave requests found</p>
          </CardContent>
        </Card>
      )}
    </div>
