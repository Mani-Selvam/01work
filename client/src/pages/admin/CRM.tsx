...
export default function CRM() {
  ...
  const sections = [
    ...
    <div key="enquiry-list" className="min-w-full flex-shrink-0 p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-2xl font-bold" data-testid="text-enquiry-list-title">Enquiry List</h2>
        <Button
          onClick={() => {
            setSelectedEnquiry(null);
            setCurrentSection(2);
              ...
              <TableBody>
                {filteredEnquiries.map((enq) => (
                  <TableRow key={enq.id} data-testid={`row-enquiry-${enq.id}`}>
                    ...
                    <TableCell>
                      <span className="capitalize">{enq.status.replace("_", " ")}</span>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => deleteEnquiryMutation.mutate(enq.id)}
    ...
    <div key="enquiry-form" className="min-w-full flex-shrink-0 p-6 space-y-6">
      <h2 className="text-2xl font-bold" data-testid="text-enquiry-form-title">
      ...
      <Card>
        <CardContent className="p-6">
          <form key={selectedEnquiry?.id || 'new'} onSubmit={handleEnquirySubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="productName"
                  name="productName"
                  defaultValue={selectedEnquiry?.productName || ""}
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="productVariant"
                  name="productVariant"
                  defaultValue={selectedEnquiry?.productVariant || ""}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="date"
                  required
                  defaultValue={selectedEnquiry?.enquiryDate || format(new Date(), "yyyy-MM-dd")}
              ...
              <div className="col-span-full space-y-2">
                ...
                <Textarea
                  id="address"
                  name="address"
                  defaultValue={selectedEnquiry?.address || ""}
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="submit" data-testid="button-submit-enquiry">
              ...
              <TableBody>
                {followups.map((followup) => (
                  <TableRow key={followup.id} data-testid={`row-followup-${followup.id}`}>
                    ...
                    <TableCell>
                      <span className="capitalize">{followup.enquiryStatus.replace("_", " ")}</span>
                    </TableCell>
                    <TableCell>{followup.nextFollowupDate || "N/A"}</TableCell>
                    <TableCell className="max-w-xs truncate">{followup.remark}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
    ...
    <div key="followup-form" className="min-w-full flex-shrink-0 p-6 space-y-6">
      <h2 className="text-2xl font-bold" data-testid="text-followup-form-title">Add New Followup</h2>
      ...
      <Card>
        <CardContent className="p-6">
          <form key={followupFormKey} onSubmit={handleFollowupSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select key={`enquiry-${followupFormKey}`} name="enquiryId" required>
                  ...
                  <SelectContent>
                    {enquiries.map((enq) => (
                      <SelectItem key={enq.id} value={String(enq.id)}>
                        {enq.customerName} - {enq.mobileNo}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                ...
                <Input id="payment" name="payment" data-testid="input-payment" />
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="submit" data-testid="button-submit-followup">
                Create Followup
              </Button>
              <Button
                type="button"
    ...
    <div key="reports" className="min-w-full flex-shrink-0 p-6 space-y-6">
      <h2 className="text-2xl font-bold" data-testid="text-reports-title">CRM Reports</h2>
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-muted-foreground">
            <p>Reports section coming soon...</p>
            <p className="text-sm mt-2">Track sales performance, conversion rates, and lead analysis.</p>
          </div>
        </CardContent>
      </Card>
    </div>,
